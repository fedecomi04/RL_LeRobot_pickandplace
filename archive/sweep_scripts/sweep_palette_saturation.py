# 10 palette variants sweeping saturation, all at a fixed lower brightness
# (val_lift=0.30) and with orange hue nudged to 28° so it stays distinct
# from red. Brightness reduced from the earlier anchor because the real
# cubes are a little darker than the boosted render.
...
VAL_LIFT     = 0.30   # darker than anchor (0.50)
ORANGE_HUE   = 28.0   # degrees — keeps orange visibly separate from red
...
SAT_LIFTS = [0.20, 0.30, 0.40, 0.50, 0.55, 0.60, 0.70, 0.80, 0.90, 0.97]
...
def main():
    ...
    for i, s in enumerate(SAT_LIFTS):
        print(f"[{i+1}/{len(SAT_LIFTS)}] sat_lift={s:.2f} (val_lift={VAL_LIFT}, orange@{ORANGE_HUE}°)",
              flush=True)
        ...
        panels.append(scb.label_panel(big_bgr,
            f"#{i+1}  sat_lift={s:.2f}  (val_lift={VAL_LIFT}, orange@{int(ORANGE_HUE)}°)"))
    ...
    out_path = os.path.join(out_dir, "palette_saturation.png")
    cv2.imwrite(out_path, full)
    print(f"\nSaved: {out_path}  ({full.shape[1]}w × {full.shape[0]}h)")

    print("\n" + "=" * 70)
    for s, palette in palettes_for_log:
        print(f"\n# sat_lift={s:.2f}  val_lift={VAL_LIFT}  orange_hue={ORANGE_HUE}°")
